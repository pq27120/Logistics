...
public class CargoPresenter {
    ...
    public void initCargoList() {
        ...
        String userName = CommonTool.getSharePreference((Context) cargoView, "curUserName");
        List<Customer> customerList = customerDao.queryBuilder().where(CustomerDao.Properties.Status.eq("1"), CustomerDao.Properties.UserName.eq(userName))
                .list();
